...
<center><h2>品牌管理<a style="float:right;" href="{{url('/weblist')}}" class="btn btn-success">列表</a></hr></h2><hr/></center>
	...
	@endif -->
<form action="{{url('/weblist/store')}}" method="post" class="form-horizontal" role="form" enctype="multipart/form-data">
@csrf
<!-- {{csrf_field()}}
...
<div class="form-group">
		...
		<div class="col-sm-10">
			<input type="text" class="form-control" name="web_name" id="firstname" 
				   placeholder="请输入网站名称">
			<b style="color:red">{{$errors->first('web_name')}}</b>
		</div>
	</div>
	<div class="form-group">
		<label for="lastname" class="col-sm-2 control-label">网站网址</label>
		<div class="col-sm-10">
			<input type="text" class="form-control" name="web_url"  id="lastname" 
				   placeholder="请输入网站网址">
				   <b style="color:red">{{$errors->first('web_url')}}</b>
		</div>
	</div>
    <div class="form-group">
    ...
    <div class="form-group">
		...
		<div class="col-sm-10">
			<input type="text" class="form-control" name="web_tel"  id="lastname" 
