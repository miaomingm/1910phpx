...
class LoginController extends Controller
{
    ...
    public function loginDo(){
        ...
        //       return redirect("/goods")->with("msg","用户名或密码有误");
        // }
        $arr=["reg_name"=>$reg['reg_name'],'reg_id'=>$reg_name['reg_id']];
        $arr=serialize($arr);
        //dd($arr);
        $sio= session(['user'=>$arr]);
    ...
    public function regDo(){
        ...
        //验证手机号/邮箱号
        if($post["reg_name"]==""){
            return redirect("/reg")->with("msg","手机号或邮箱号必填");exit;
        }else{
            $where=[
            ...
            //如果查询到的条数大于0，代表已被注册
            if($count>0){
                return redirect("/reg")->with("msg","手机号或邮箱号已被注册");exit;
            }else if($sessionInfo["send_name"]!=$post["reg_name"]){
                return redirect("/reg")->with("msg","发送验证的号与注册的号不一致");exit;
            }
        }
        //短信验证码
        if($post["reg_code"]==""){
            return redirect("/reg")->with("duan","验证码必填");exit;
        }else if($sessionInfo["send_code"]!=$post["reg_code"]){
            return redirect("/reg")->with("duan","验证码错误");exit;
        }else if(time()-$sessionInfo['send_time']>300){
            return redirect("/reg")->with("duan","验证码已失效");exit;
        }
        //密码
        $pwd='/^[0-9a-z]{6,18}$/i';
        if($post["reg_pwd"]==""){
            return redirect("/reg")->with("mi","密码必填");exit;
        }else if(!preg_match($pwd,$post["reg_pwd"])){
            return redirect("/reg")->with("mi","密码错误");exit;
        }
        //确认密码
        if($post["pwd"]!=$post["reg_pwd"]){
            return redirect("/reg")->with("qmi","确认密码必须与密码一致");exit;
        }
        // dd($post);
        ...
        $res=Reg::insert($post);
        if($res){
            return redirect("/login")->with("zhu","注册成功，请登录");
        }else{
            return redirect("/login")->with("zhu","注册失败，请重新注册");
        }
    }
    //手机号
    ...
    public function sendByMobile($mobile,$code){
        ...
        AlibabaCloud::accessKeyClient('LTAI4G4VXsZuxeEzV4wjntem', 'DSU7BCwapFqLmzpHcitEm4biYpHPzx')
